---
pagetitle: "Dapla-test"
page-layout: custom
section-divs: false
toc: false
css: index.css
editor: source
description: | 
  A manual for using Statistics Norways data platform Dapla.
hide-description: true
image: images/dapla-logo.png
resources: 
  - images/animation.mp4
---

::: {.hero-banner}


::: {.content-block}

::: {.hero-text}
# Dapla-manualen

### Brukermanualen for SSBs dataplattform Dapla

- Lær om Dapla-team
- Utvikle kode i R eller Python
- Velg programmeringsmiljøer som [Jupyterlab](https://jupyter.org/), [VSCode](https://code.visualstudio.com/docs/editor/vscode-web) eller [RStudio](https://posit.co/products/open-source/rstudio-server/)
- Utnytt prosesseringskraften i et skalerbart [Kubernetes](https://kubernetes.io/)-miljø
- Reproduserbare resultater med Git og GitHub
- Uendelig med lagringsplass i *Google Cloud Storage* bøtter

### Hvordan? Hva? Hvorfor? Dapla-manualen gjør overgangen til Dapla enklere.

::: {.hero-buttons}
[For statistikkere](./statistikkere/hurtigstart.qmd){.btn-action-primary .btn-action .btn .btn-success .btn-lg role="button"}
[For utviklere](./utviklere/index.qmd){#btn-guide .btn-action .btn .btn-info .btn-lg role="button"}
::: 
:::

::: {.hero-animation}

<video autoplay muted playsinline loop>
<source src="images/animation.mp4" type="video/mp4"/>
</video>

:::

:::


:::


::: {.hello-quarto .alt-background}
::: {.content-block}


# Velkommen   


Dapla-manualen tar sikte på å gi SSB-ansatte mulighet til å ta i bruk grunnleggende funksjonalitet på Dapla uten hjelp fra *eksperter*. Den skal hjelpe statistikkere, forskere og utviklere med å finne relevant informasjon på en enkel måte. Manualen er bygget opp med en del for [statistikkere/forskere](./statistikkere/index.html), og en del for [IT-utviklere](./utviklere/index.html). I tillegg har vi en [blogg](./blog/index.html) der alle i SSB kan bidra med erfaringer, tanker og triks som kan være nyttig i overgangen til en ny plattform.   
<br>

::: {.grid}


:::
:::
:::


::: {.content-block}
::: {.features}

::: {.feature}
### Dapla-team
For å jobbe på Dapla trenger man et Dapla-team. Det er teamet som får tilgang til ressurser på plattformen slik at de kan jobbe med data. 

::: {.learn-more}
[Les mer »](statistikkere/dapla-team.html)
:::
:::

::: {.feature}
### R og Python
På Dapla støttes programmeringsspråkene R og Python. Det er kraftige verktøy for å jobbe med data og statistikk. 

::: {.learn-more}
[Les mer »](statistikkere/jobbe-med-kode.html)
:::
:::

::: {.feature}
### Metadata
I SSB utvikles det et helhetlige metadatasystemer som skal gjøre det enklere å finne frem til data og forstå dem.

::: {.learn-more}
[Les mer »](statistikkere/metadata-overview.html)
:::
:::

::: {.feature}
### Git og GitHub
På Dapla brukes Git til å versjonhåndtere kode og GitHub til å lagre koden. 

::: {.learn-more}
[Les mer »](statistikkere/git-og-github.html)
:::
:::

::: {.feature}
### Programmeringsmiljøer
Velg mellom moderne programmeringsmiljøer som Jupyterlab, VSCode eller RStudio. 

::: {.learn-more}
[Les mer »](statistikkere/kode-overview.html)
:::
:::

::: {.feature}
### ssb-project
Følg beste-praksis for koding i SSB med vårt interne verktøy ssb-project.

::: {.learn-more}
[Les mer »](statistikkere/jobbe-med-kode.qmd)
:::
:::

:::
:::

::: {.get-started .alt-background}
::: {.content-block}

[Kom i gang](statistikkere/hurtigstart.html){.btn-action-primary  .btn-action .btn .btn-success .btn-lg role="button" style="margin-top: 1em;"}

:::
:::

