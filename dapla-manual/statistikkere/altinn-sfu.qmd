...
|------------|------|-------------|
| `pulje` | string | Filtrer på én eller flere puljer. Angi puljenummer som kommaseparert streng, f.eks. "1" eller "1,2". Verdien "0" brukes for å hente enheter uten pulje. "0" kan ikke kombineres med andre puljer. Dersom parameteren ikke oppgis, returneres hele utvalget uavhengig av pulje. |
...
| Verdi for `pulje` | Resultat                              |
| ----------------- | ------------------------------------- |
| `"1"`             | Hent kun pulje 1                      |
| `"1,2"`           | Hent pulje 1 og 2                     |
| `"0"`             | Hent kun enheter uten pulje      |
| *(ikke oppgitt)*  | Hent hele utvalget uavhengig av pulje |

**Merk:** Verdien "0" kan ikke kombineres med andre puljer. For eksempel er "0,1" ikke gyldig.
...
```json
{
    ...
    "orgnr": "123456789",
    "enhets_type": "FRTK",
    "reportee_ident_nr": "A3TF0001",
    "reportee_orgnr": "123456789",
    "reportee_enhets_type": "FRTK",
    "foretak": "A3TF0001",
    ...
    "dato_innkvittering": "2025-12-29T11:51:18.000",
    "kvitt_type": "K",
    "kvitt_format": "A",
...
For å hente enhetsinformajon fra SFU, bruk metoden `get_enhet_from_sfu` i `SuvClient`. Sørg for at du oppgir riktig delregisternummer og organisasjonsnummer.
...
For å generere prefill fra ISEE brukes metoden `initialize_prefill_isee` i `SuvClient`. Metoden oppretter en jobb som genererer prefill-data og lagrer resultatet i `DYNAREV.VW_PRE_SKJEMA_DATA`. Sørg for at du oppgir riktig delregisternummer og RA-nummer.

```{.python filename="notebook"}
client = SuvClient()

response = client.initialize_prefill_isee(
    delreg_nr=190125,
    ra_nummer="RA-0484A3" 
)
...
#### Hente generert prefill

Når genereringen er fullført kan prefill hentes med metoden `fetch_prefill_isee` i `SuvClient`. Sørg for at du oppgir riktig delregisternummer og RA-nummer. 

Som standard returneres kun prefill-data som er generert de siste 24 timene. Dersom dataene er eldre må enten prefill genereres på nytt, eller parameteren `max_age_minutes` overstyres. 

Standardverdi for `max_age_minutes` er `1440` minutter (24 timer).

```{.python filename="notebook"}
client = SuvClient()

response = client.fetch_prefill_isee(
    delreg_nr=190125,
    ra_nummer="RA-0484A3" 
)
...
| Parameter | Type | Beskrivelse |
|------------|------|-------------|
| `max_age_minutes` | int | Maks alder på prefill-data i minutter. Standard er `1440` (24 timer) |
| `ident_nr` | str | Hent spesifikk enhet |
| `enhets_type` | str | Hent spesifikk enhetstype |
| `felt_ids` | list[str] | Hent bare spesifikke felt |
| `max_results` | int | Maks antall resultater som returneres |
