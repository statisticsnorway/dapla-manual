...
Administrasjon av utvalg og enheter skjer fortsatt fra SFU på bakke. Knytningen mellom skjemaet og bakkesystemene skjer ved at et delregister-nr registreres på perioden. Mer informasjon om håndtering av utvalg fra SFU finner du [her](./altinn-sfu.html).
...
| Journalnummer | Unikt journalnummer knyttet til undersøkelsen.
: Periode metadata {#tbl-periode-meta .striped}
:::

### Kodeeksempler

Nedenfor finner du eksempler på hvordan du kan opprette og hente perioder ved hjelp av `SuvClient` i Python.

#### Opprette en ny periode

For å opprette **en ny periode**, bruk metoden `create_periode` i `SuvClient`.
...
#### Hente eksisterende perioder

For å **hente ut eksisterende perioder** med tilhørende metadata i Python, bruke metoden `get_perioder_by_skjema_id`. Pass på at du oppgir riktig skjema_id.
...
#### Opprette flere nye perioder (med puljer og utsendinger)

Bruk metoden `create_new_periodes` for å **opprette en eller flere nye perioder** , inkludert eventuelle puljer og utsendinger.

Perioden som brukes som utgangspunkt må være siste registrerte periode, og du må angi hvor mange nye perioder som skal opprettes.
...
print(json.dumps(output, indent=4))
```
Systemet oppretter nye perioder automatisk og setter datoer for puljer og utsendinger basert på periodetype. Kontroller datoene nøye og juster dem ved behov. Delregister-nr må oppdateres manuelt for alle nye perioder.
...
| Send SI | Dato for innsending til Statens innkrevingssentral  
