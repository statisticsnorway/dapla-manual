...
2. Bytte mappe til repoet `cd <repo-navn>`
3. Opprett en ny branch: `git switch -c add-kildomaten-source`
...
Skal du sette opp kildomaten i prod-miljøet kan du følge oppskriften som kommer senere i kapitlet uten å gjøre noe mer enda. Vær oppmerksom at mappe navnet kan ikke være lengere enn 23 tegn. 
...
`ssb-dapla-example-data-kilde-prod/altinn/`.
...
3. Python-pakkene som kan benyttes [er definert her](https://github.com/statisticsnorway/dapla-automation-processor/blob/main/source_data/pyproject.toml). Ved behov for andre pakker, [ta kontakt med Kundeservice](https://ssb.pureservice.com/#/categories/14/new).
4. [Bøttemontering](hva-er-botter.qmd#bøttemontering-med-cloud-storage-fuse) gjelder ikke for kildomaten, så man må bruke pakken `gcsfs` når man gjør filoperasjoner i kildomaten.
:::
...
`gs://ssb-dapla-example-data-kilde-prod/altinn/test20231010.csv` hvis filen `test20231010.csv` dukker opp i `ssb-dapla-example-data-kilde-prod/altinn/`.
...
`automation/source-data/dapla-example-prod/altinn/`. Senere, når vi har rullet ut tjenesten, vil en ny fil i `gs://ssb-dapla-exmaple-data-kilde-prod/altinn/` trigge kildomaten og kjøre koden i `process_source_data.py` på filen.
...
1. *Push branchen* til GitHub og opprette en *pull request*. Se hvordan på punktet 4 [her](https://manual.dapla.ssb.no/statistikkere/git-arbeidsflyt.html#lagre-kode-og-oppdatere-repoet-p%C3%A5-github).
...
- Bruker `process_source_data.py` biblioteker som ikke [er installert i kildomaten](https://github.com/statisticsnorway/dapla-automation-processor/blob/main/source_data/pyproject.toml)?
...
`gs://ssb-dapla-example-data-kilde-prod/altinn/`
...
2. **folder_prefix** er mappen i kilde-bøtten, det vil si, mappen akkurat under `<dapla-example-prod>`. Man vil da kjøre Kildomaten-scriptet for hver fil i denne mappen.
...
`ssb-dapla-example-data-kilde-prod/altinn/ra0678/`.
...
`ssb-dapla-example-data-kilde-prod/altinn/ra0678/data1.parquet`, `ssb-dapla-example-data-kilde-prod/altinn/ra0678/data2.parquet` og `ssb-dapla-example-data-kilde-prod/altinn/ra0678/data3.parquet`.
