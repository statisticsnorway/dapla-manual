...
::: {.callout-caution}
NB! syntaks highlightning kan variere, farger på yaml kode er kun veiledende
::: 

Alle yaml-filer følger samme struktur. 
...
```
Dette er en yaml-fil som genereres for å opprette en ny variabeldefinisjon. 
...
Dette er en yaml fil fra lagret variabeldefinisjon
...
```
Beskrivelser 
som er basert på modell beskrivelse ref
Lenke modell
```{.YAML .code}
...
Eksempler 
Tydeligjøre format og hvilken verdi som forventes.
...
#### Enkle verdier
::: {.callout-note}
Format er `DoubleQuotedScalar`. All tekst må være omkranset av doble fnutter. `""`
:::

Beregnet for kort tekst som ikke skal lagre formattering ikke skal lagres.
Feltet `kortnavn`:
```{.YAML .code}
short_name: "landbak"
...
#### Flerspråklige felt
::: {.callout-caution collapse=True}
##### Flerspråklige felt har nøstede nøkler. 
```{.YAML .code}
contact:
    title:
        nb: |-
...
::: {.callout-warning}
Streng indentering. Hvert listeelement må.
::: 
