...
Eksemplene som vises er for å laste produksjonsdata, dersom du ønsker å sende ned skjemaer fra test-altinn (tt02) til DB1T kan du se på tegningen av den overordnede flyten.
    - Prosessen er identisk, men filstiene er ulike.
    - Det anbefales å sette opp test-løpet, så du slipper å potesnielt lage rot i DB1P.

For å sette opp testløpet er det foreløpig nødvendig å kontakte kundeservice for å åpne litt på tilganger, slik at du kan overføre filer altinn-staging til prod teamet ditt. Til vanlig så har du kun tilgang til altinn-staging fra test-teamet, men siden test-teamet ikke kan overføre filer til bakken så er det ikke mulig å teste innlasting til ISEE uten å bruke prod-teamet.
...
Før vi går detaljert gjennom hvert steg så kan det være greit å få et overblikk av hva vi skal begi oss ut på. Helt overordnet ser flyten som skal settes opp slik ut

![Mappestruktur på cloud_sync](../images/altinn3-isee-flow.svg)
