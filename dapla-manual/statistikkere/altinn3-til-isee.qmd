...
Eksemplene som vises er for å laste produksjonsdata, dersom du ønsker å sende ned skjemaer fra test-altinn (tt02) til DB1T kan du se på tegningen av den overordnede flyten.
    - Prosessen er identisk, men filstiene er ulike. Se skjermbilde for flyten og eget avsnitt.
...
I prosessen er det en del overføringer som må settes opp, men de kjøres automatisk når det først er i gang. Stegene vi skal gjennom er:
    ...
    6. Lasting til ISEE
    7. Prosessen er satt opp, hva nå?

Testløpet krever at du gjør alle stegene nedenfor dobbelt opp. Én gang for prod, én gang for test. Det er viktig å ikke blande disse løpene i hverandre, og at filstiene skrives henvisning til prod eller test mappen. Se flytdiagrammet nedenfor.

::: {.callout-important}
## Overføring mellom altinn-staging og prod

For å sette opp testløpet er det foreløpig nødvendig å kontakte kundeservice for å åpne på tilganger, slik at du kan overføre filer altinn-staging, datamottaket som skjemaer fra test-altinn / tt02 havner, til prod teamet ditt.

Til vanlig så har du kun tilgang til altinn-staging fra test-teamet, som fungerer på Dapla, men siden test-teamet ikke kan overføre filer til bakken så er det ikke mulig å teste innlasting til ISEE med skjemaer fra tt02 uten å gå via prod teamet ditt. Derfor trenger du at det åpnes for dette.
:::



![Flytdiagram for prosessen](../images/altinn3-isee-flow.svg)
...
Siden det første steget involverer kildebøtta så trenger du å være data-admin på teamet ditt (du kan sjekke rollene dine i dapla ctrl https://dapla-ctrl.intern.ssb.no/) og søke om midlertidig tilgang (https://jitaccess.dapla.ssb.no/).
...
Deretter skal du klikke øverst til venstre, like til høyre for GoogleCloud og få opp menyen hvor du velger hvilket prosjekt du skal jobbe med

Her skal du velge ditt team, du kan se teamnavnet i dapla-ctrl. Du skal inn på prosjektet som har ditt-teamnavn*-kilde-p* i navnet sitt.

![Skjermbilde som viser hvor du skal velge prosjekt](../images/altinn3-isee-finne-prosjekt.png)

Når du er inne på prosjektet ditt kan du klikke i søkefeltet på toppen av siden og søke på "Storage transfer" og velge den.
...
Mappestrukturen i altinn-data-prod er slik at det først er en mappe for året skjemaet ble mottatt, deretter for måneden, deretter for dagen, og her finner du hver enkelt innsending.
    - I transfer job henviser vi bare til den øverste mappen, den fører automatisk med seg alt som ligger lenger "nede" i mappestrukturen.

Se på bildene for å se hvordan innstillingene for jobben skal se ut.
...
Et lite tips er å la denne jobben kjøre ofte, siden det vil gjøre at skjemaene behandles fortløpende i kildomaten istedenfor å komme i store overføringer.
    - Andre overføringer trenger ikke kjøre mer enn en gang om dagen.
    - Her skal ikke filer slettes. 
...
For mer informasjon om kildomaten kan du se her https://manual.dapla.ssb.no/statistikkere/kildomaten.html
    - Se spesielt på "Rull ut tjenesten"
...
Først skal du skifte hvilket prosjekt du jobber i. Nå skal vi være i teamet som heter ditt-teamnavn-p, altså det som ikke har kilde i navnet sitt.
...
Først skal du opprette skjemaet i SFU og ISEE. Det skal hete RA-XXXXA3.
...
MoveIt som flytter fra: <- Denne jobben trenger du bare hvis du skal sette opp test-løpet også.
...
Det er mange detaljer som skal stemme for at dataene som lastes inn skal dukke opp riktig i ISEE. Du må sjekke at kodelister eksisterer for variablene dine, at disse matcher variabelverdiene som kommer fra altinn3 og at eventuelle endringer i altinn3 skjemaet er hensyntatt i skjemavisningen i ISEE.
    - Ved behov kan du opprette nye kodelister som passer altinn3 dataene dine i klass-forvaltning.
...
Det er mulig å abonnere på rapporter fra ISEE også hvis du går inn på Administrasjon -> Oversikt datafangst -> Datafangstrapporter -> Legg inn epostadressen din.
    - Du kan også få varsler om noe skulle gå galt underveis i lastingen til ISEE.

Her er det en samling av ressurser som kan være til hjelp med å ferdigstille opplegget ditt:
