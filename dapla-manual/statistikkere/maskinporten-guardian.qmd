...
2) Når data er delt av en API-tilbyder, og en har navnet på API scopes, kan [M2M-teamet hos SSB](mailto:Team-M2M@ssb.no) kontaktes
...
Eksempelet under bruker Python-biblioteket [ssb-guardian-client](./ssb-guardian-client.qmd).
...
# Hent et Maskinporten-token
maskinporten_access_token = client.get_maskinporten_token(
    maskinporten_client_id= config[api_name]['maskinporten_client_id'],
    scopes= config[api_name]['scopes']  # Liste av scopes
)
...
response = requests.get(
    ...
    headers={"Authorization": f"Bearer {maskinporten_access_token}"}
)
