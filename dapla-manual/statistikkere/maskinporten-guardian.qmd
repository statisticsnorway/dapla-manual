...
::: {.callout-tip appearance="simple"}
Alle med en SSB-epostadresse kan [registerere]((https://user.difi.no/auth/realms/difi/login-actions/registration?client_id=samarbeid-lukket&tab_id=9q0RtrwC2Vo))
...
The following code example expects a toml config file to exist named
<team_uniform_name>-<env>-maskinporten-personal.toml (e.g. play-foeniks-prod-maskinporten-personal.toml)
...
# Check which environment we're running and load corresponding config
dapla_env = os.getenv("DAPLA_ENVIRONMENT").lower()
config = toml.load(f"{team_uniform_name}-{dapla_env}-maskinporten-personal.toml")
