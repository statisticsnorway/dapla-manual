...
Alle statistikkteam skal følge navnestandarden for lagring av [datatilstander](./datatilstander.html) på Dapla. Standarden gjelder i alle kilde-, produkt- og delt-bøttene som teamet måtte ha, så lenge datasettet er i en av de definerte datatilstandene [inndata](./datatilstander.html#inndata), [klargjorte data](./datatilstander.html#klargjorte-data), [statistikk](./datatilstander.html#statistikk) og [utdata](./datatilstander.html#utdata)^[Datatilstanden “kildedata” omfattes ikke av denne navnestandarden i og med at kildedata mottas av SSB i mange former/strukturer, og at kildedata i
liten grad skal deles internt og eksternt (begrenset tilgang). Det må eventuelt vurderes om det skal utarbeides egne retningslinjer for lagring av kildedata. Datatilstanden “inndata” er nevnt i parentes i og med at denne datatilstanden ikke er obligatorisk.]. Navnestandarden gjelder foreløpig kun for lagring av datafiler i bøttene Google Cloud Storage og ikke i alternative lagringsressurser som *BigQuery* og *CloudSQL*.

```{.bash filename="Mappestruktur for team dapla-example"}
ssb-play-obr-data-produkt-prod
|
└─ statistikkprodukt1
|  |
|  └── klargjorte
