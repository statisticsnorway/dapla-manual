...
Data i de permanente [datatilstandene](./datatilstander.html) inndata, klargjorte data. statistikkdata og utdata skal lagres i Google Cloud Storage (GCS) bøtter og følge en definert navnestandard. Standarden gjelder for både statistikkprodukter^[fkdjfkdjs] og dataprodukter^[dfkjsdkfjklds]. Navnestandarden beskrevet i dette kapitlet er derfor gjeldende for alle data som lagres i bøtter i **standardprosjektet**, som f.eks. **produktbøtta** og **delt-bøttene**. 

::: {.callout-note}
## Gjelder navnstandarden for kildedata?

Navnestandarden er obligatorisk for datatilstandene *inndata*, *klargjorte data*, *statistikkdata* og *utdata*. Datatilstanden *kildedata* omfattes ikke av navnestandarden. Grunnen er at kildedata mottas av SSB i mange former/strukturer og de deles sjelden med andre team. De unike egenskapene til kildedata er også grunnen til at de ikke har samme krav til dokumentasjon i metadatasystemene heller.  
:::
...
1. Statistikkens kortnavn eller dataproduktets navn
2. Datatilstand
...
- Teamet trenger å midlertidig lagre temporære data. Dette kan gjøre i en temp-mappe på første nivå etter bøttenavn. 
...
Under er et nytt eksempel i produktbøtta for team **dapla-example**, men nå har de kun statistikkproduktet **ledstill**, en **temp**-mappe og en **oppdrag**-mappe. I tillegg så ønsker de å skille mellom data som produsert på Dapla og data som er migrert fra tidligere plattform. De gjør det ved å opprette de egendefinerte mappene **on-prem** og **dapla** for hver datatilstand.
...
Når man oppretter en mappe for oppdrag så er det viktig å kunne knytte dataene til et Websak-saksnummer. Det er derfor anbefalt at en undermappe med saksnummeret eller ha det med i filnavnet. 
:::
...
Eksempelet i @fig-filename-breakdown har **varehandel** som kort beskrivelse, dataene er gyldige for **2018Q1**, versjon er **1** og filtypen er **parquet**. I tillegg ser vi at periodeangivelse alltid skal prefixes med **p** og versjon med **v**. 

Det er også verdt å merke seg at at mellomrom og særnorske bokstaver som **æ**, **ø** og **å** ikke forekommer i filnavnet. Følgende alfanumeriske tegn kan benyttes i fil- og mappenavn:   
...
Anta at det er team som heter **dapla-example** med statistikkproduktene ledstill og sykefra, og de har et dataprodukt med navn ameld. Teamet har følgende mappestruktur i produktbøtta:
...
Team som partisjonerer sine filer ved lagring skal fortsatt følge navnestandarden. Det som endrer seg er at **filtype** ikke blir en del av filnavnet, men heller kommer inn under partisjoneringen. Anta at team **dapla-example** partisjonerer et datasett i datatilstand inndata som heter **varehandel_p2018Q1_v1**. Anta også at de partisjonerer dataene med hensyn på kolonnen **aar**. Da vil de hatt en slik struktur:

```{.yaml filename="Produktbøtta"}
ssb-dapla-example-data-produkt-prod/  
└─ ledstill/  
   ├── inndata/
        └── varehandel_p2018Q1_v1
            └── aar=1980
                └── data.parquet
            └── aar=1981
                └── data.parquet
