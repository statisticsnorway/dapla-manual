Manualen er verbose, førte til en jobb som ikke kjørte. Og eksempelet var fra sky til bakke og ikke motsatt.
--------------
# Ny
--------------
---
title: Hurtigsporet: Overføre fra bakke til sky
freeze: true
number-sections: true
---

# Overføre fra bakke til sky

<details><summary><strong>Nødvendige tilganger</strong></summary> 
- Tilgang til server på bakke(omtalt som Linux)
    - Datatilgang til data
    - Tilgang til mapper i prodsonen
    - Tilgang til cloud sync
- Tilgang til Teamet på dapla (kan sjekkes i [dapla CTRL](https://dapla-ctrl.intern.ssb.no/))
</details>

<details><summary><strong>Prerequisites</strong></summary> 
- Du må huske kortnavnet til teamet se (kan sjekkes i [dapla CTRL](https://dapla-ctrl.intern.ssb.no/))
- Statestikken din må ha et kortnavn
</details>



## **Sett opp tilgang for kildebøtten i JIT**

I [JIT-applikasjonen](https://jitaccess.dapla.ssb.no/) må du gi tilgang for kildebøtten, for å sette opp overføringer dit, produktbøtten er ikke like streng da trenger du ikke å gjøre dette.
- Aktiver rollene **storagetransfer.admin**, **ssb.buckets.list** og **ssb.bucket.write**.^Du må ha data tilgang til kildebøtten, sjekk i [Dapla Ctrl](https://dapla-ctrl.intern.ssb.no/) om du ikke har tilgang og kontakt kundeservice.
- Hvor lenge du ønsker tilgangen. Til slutt oppgir du en begrunnelse for hvorfor du trenger tilgangen og trykker **Request access**. 
Når du har gjort dette vil du få en bekreftelse på at tilgangen er aktivert, og det tar ca 1 minutt før den aktiverte tilgangen er synlig i GCC.^[Les mer om JIT-applikasjonen her](./appendix/jit.html). 

[Google Cloud Console (GCC)/transfer/jobs](https://console.cloud.google.com/transfer/jobs) - Brukergrensesnitt for å sette opp og administrere overføringene.^[I SSB kaller vi tjenesten for *Transfer Service*, men du kan oppleve at Google kaller den litt forskjellige ting. Den blir omtalt som *[Storage Transfer Service](https://cloud.google.com/storage-transfer-service)* noen steder, mens i *Google Cloud Console* blir den omtalt som *Data Transfer* eller *Storage Transfer*].


## **Overføring**

Overføringer skjer i to steg. 
''' merkle
Bakke -> Linux
Linux -> Sky
'''
### Bakke -> Linux
På [Bakke](https://portal.ssb.no/Citrix/FAS-SSB-CitrixWeb/) -> Filezilla -> fil -> Site manager -> sl-sas-work-1 -> Logon type: Ask for password -> user: dine_initialer -> connect -> Local site: kopier inn filsti fra mappen eller naviger -> Remote site: /ssb/cloud_sync -> team navn -> kilde/standard(som er prod) -> tilsky -> Dra filene du skal ha over til tilsky mappen

### Linux -> Sky
På Pc -> [Google Cloud Console (GCC)/transfer](https://console.cloud.google.com/transfer) -> (ctrl + o) eller open project picker i øvre venstre (select project) -> skriv *teamets_prosjektnavn* -> velg: teamets_prosjektnavn-kilde-p eller teamets_prosjektnavn-p -> + create transfer job -> 

Source type: POSIX filesystem -> Destination type: Gogle Cloud Storage -> Next step -> 

Agent pool: transfer_service_deafult -> tilsky -> Next step -> 

Browse: Velg tilsky -> Next step -> 

run once + starting now -> Description: f.eks " Legger opp til Dapla" -> velg: "Delete files from source after they're transferred" så slipper du å slette gamle filer. -> Bla til toppen -> Create

### Logge inn på dapla


### Skriptet inneholder funksjonene du trenger i ulike celler. 
Du kan bruke monterte bøtter, da skal mappene vises i systemet. Eller så kan du bruke gscfs som kan bruke vanlige linux kommandoer.
```python
    # %%
    import gcsfs
    fs = gcsfs.GCSFileSystem()
    gcs_shortname = "" # Skriv inn kortnavn
    dapla_team = ""     # Skriv inn teamnavn
    # %%
    gcs_tilsky_bucket = f"gs://ssb-{dapla_team}-data-tilsky-prod"
    print(fs.tree(gcs_tilsky_bucket))  # Se hvordan mappen ser ut
    # %%
    gcs_prod_bucket = f"gs://ssb-{dapla_team}-data-produkt-prod"
    print(fs.ls(gcs_prod_bucket))  # Se hvilke kortnavn det alt er der
    # %%
    gcs_inndata = f"{gcs_prod_bucket}/{gcs_shortname}/Inndata"
    print(fs.tree(gcs_inndata))  # Se hvordan mappen ser ut
    # %%
    fs.mv(gcs_tilsky_bucket,f"{gcs_inndata}/temp",recursive=True) # Flytter filene til inndata, temp, fordi inndata skal inneholde parquetfiler. Husk å slett temp, senere.
    #fs.mv(gcs_tilsky_bucket/enfil.txt,gcs_inndata) # Flytter en fil til inndata
    #fs.put() kopier, fs.cp() kopier
    print("Filene er flyttet.")
```
...
Google Cloud Console (GCC) - Brukergrensesnitt for å sette opp og administrere overføringene. [I SSB kaller vi tjenesten for *Transfer Service*, men du kan oppleve at Google kaller den litt forskjellige ting. Den blir omtalt som *[Storage Transfer Service](https://cloud.google.com/storage-transfer-service)* noen steder, mens i *Google Cloud Console* blir den omtalt som *Data Transfer* eller *Storage Transfer*].

<details><summary><strong>Feilsøking: Første gang med gamle repo </strong></summary> Dette gjøres automatisk ved nye opprettelser, men kan være greit å sjekke om noe ikke virker.

Sjekk at tjenesten er aktivert for teamet. Transfer Service er en såkalt [feature](./features.html) som teamet kan skru av og på selv. For å sjekke om den er *skrudd på* går du inn i teamets IaC-repo^[Du finner teamets IaC-repo ved å gå inn på <https://github.com/orgs/statisticsnorway/repositories> og søke etter ditt teamnavn og åpne den som har navnestrukturen teamnavn`-iac`. For eksempel vil et team som heter **dapla-example** har et IaC-repo som heter `dapla-example-iac`.] og sjekker filen `./infra/projects.yaml`. 
...
projects:
  - project_name: dapla-example
    ...
    features:
      ...
      - transfer-service
```
I filen over ser du at teamet har skrudd på tjenesten i prod-miljøet, siden `transfer-service` er listet under `features`. Hvis tjenesten ikke er skrudd på kan du lese om hvordan du skrur den på i [feature-dokumentasjonen](./features.html). </details>

## Personlig bruker vs systembruker
Når du setter opp en overføringsjobb med Transfer Service så setter du opp en jobb som kjøres av en systembruker^[Systembrukere heter *Service Accounts* på engelsk og blir ofte referert til som *SA*-er i dagligtale.] og ikke din egen personlige bruker. En konsekves av dette er at automatiske overføringsjobber vil fortsette å kjøre selv om din tilgang til dataene er midlertidig, siden det er en systembruker som kjører jobben.



--------------
# Orginal for kanibalisering
--------------
...
Overføring mellom bøtter er en overføring av data mellom to bøtter på Dapla. Fremgangsmåten er helt likt som beskrevet tidligere, men at du nå velger **Google Cloud Storage** som både kilde og destinasjon. Igjen er vi avhengig av at systembrukeren som utfører jobben har tilgang til begge bøttene som er involvert i overføringen. Default er at et team kan overføre mellom bøtter i kildeprosjektet, og at de kan overføre mellom bøtter i standardprosjektet, men aldri mellom de to. For å overføre fra kildeprosjektet til standardprosjektet, må man bruke [kildomaten](./kildomaten.qmd). Hvis du ønsker å overføre mellom bøtter i ditt prosjekt og et annet teams prosjekt, så må du be det andre teamet om å gi din systembruker tilgang til dette.
