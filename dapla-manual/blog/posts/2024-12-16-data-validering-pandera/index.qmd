...
Når det kommer til Python bruker vi i SSB som oftest [Pytest](https://docs.pytest.org/en/stable/) pakken for testing av kode, og Pandera eller [Pydantic](https://docs.pydantic.dev/latest/) pakkene for data validering.
...
**Pandera eller Pydantic?** Hvem av som bør benyttes avhenger mest av strukturen på dataene din.
...
Med pandera kan du validere både datatyper og innhold. Det er  flere måter å definere ett schema på, men jeg kommer til å vise den   anbefalte måten å gjøre det på. Den er ikke den enkleste men den er enkel nok og har likheter til Pydantic.
...
- *navn* er forventet å være string, med regel om at det ikke skal være noen manglende verdier (missing values). Dette er standard  alle regler, så det er ikke nødvendig å notere, men for demonstrasjonens skyld så gjorde jeg det her.
...
Deretter definerer vi et eget schema for statistikkdata, men noen tilhørende regler og datatyper;
